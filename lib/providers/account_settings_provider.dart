...
class AccountSettingsProvider extends ChangeNotifier {
  ...
  Future<void> loadSettings() async {
    try {
      ...
      _settings = await AccountSettingsRepository.getSettings();
    } catch (e) {
      _error = e.toString();
    } finally {
      _isLoading = false;
  ...
  Stream<void> listenSettings() {
    return AccountSettingsRepository.streamSettings().map((data) {
      _settings = data;
  ...
  Future<void> updatePersonalizedAds(bool value) async {
    ...
    _settings = _settings!.copyWith(personalizedAds: value);
  ...
  Future<void> updateBiometricLogin(bool value) async {
    if (_settings == null) return;
  ...
  Future<void> _updateBackend() async {
    try {
      await AccountSettingsRepository.updateSettings(_settings!);
    } catch (e) {
      _error = e.toString();
