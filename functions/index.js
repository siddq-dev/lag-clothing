...
const twilioAdminWhatsAppTo = defineSecret("TWILIO_ADMIN_WHATSAPP_TO");
    ...
    async (event) => {
      try {
        ...
        const customerNameValue =
          customer.displayName || "Customer";const {onRequest} = require("firebase-functions/v2/https");
const {onDocumentCreated} = require("firebase-functions/v2/firestore");
const {defineSecret} = require("firebase-functions/params");
const logger = require("firebase-functions/logger");
const {BrevoClient} = require("@getbrevo/brevo");
...
/**
 * ============================================================
 * ORDER CREATED -> SEND CUSTOMER EMAIL + WHATSAPP NOTIFICATIONS
 * ============================================================
 *
 * Firestore path:
 *
 ...
 * 5. Build the email.
 * 6. Send the email through Brevo.
 * 7. Send a WhatsApp order confirmation to the customer.
 * 8. Send a WhatsApp order notification to the admin.
 */
exports.sendOrderConfirmationEmail = onDocumentCreated(
    {
      document: "orders/{orderId}",
    ...
    async (event) => {
      try {
        ...
        /**
         * --------------------------------------------------------
         * BUILD ORDER ITEMS PLAIN TEXT (FOR WHATSAPP)
         * --------------------------------------------------------
         */
        ...
        const itemsText =
          items.length === 0 ?
            ...
            items
                .map((item) => {
                  const productName =
                    stringValue(item.productName, "Product");

                  const size =
                    stringValue(item.size, "-");

                  const color =
                    stringValue(item.color, "-");
         ...
         * --------------------------------------------------------
         *
         * Sent to both the customer and the admin. Wrapped in
         * its own try/catch per recipient so a WhatsApp failure
         * never blocks the other recipient or the email step
         * below.
         */

        const whatsAppOrderSummary =
          `🛍️ *LAG Clothing - Order Confirmed*\n\n` +
          `Order #${orderNumber}\n\n` +
          `${itemsText}\n\n` +
        ...
        if (customerPhoneNumber) {
          ...
          logger.info(
              ...
              "skipping customer WhatsApp message",
              {
                orderId,
                userId,
              },
        ...
        try {
          const adminWhatsAppNumber = twilioAdminWhatsAppTo.value();
          ...
          await sendWhatsAppMessage(adminWhatsAppNumber, adminMessage);
          ...
          logger.error(
              "Failed to send WhatsApp message to admin",
              {
                orderId,
                error: error.message,
              },
